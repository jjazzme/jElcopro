  ...
  <div>
    ...
    >
      ...
      <div
        ...
        :data-column="k2"
        v-show="value.shells.value[k2].show"
        v-if="value.shells.value[k2].hidden !== true"
      >
        <span
          @click="clickCell"
          :data-column="k2"
          :data-key="row.id"
          :class="`${ value.shells.value[k2].parentClass ? value.shells.value[k2].parentClass : '' } ${ value.shells.value[k2].to ? 'link' : '' }`"
          v-html="v2"
        ></span>
      </div>
    </div>
    ...
    methods:{
      ...
      clickCell(e){
        ...
        if (!(obj.contentEditable==='true') && $(obj).hasClass('link')){
          const name = this.value.shells.value[$(obj).attr('data-column')].to.name;
