  ...
  >
    ...
    >
      ...
      >
        <b-form-checkbox
          v-if="i.hidden!==true"
          v-for="(i,k) in value.shells.template"
    ...
    <!-- поля заголовка -->
    <div
      v-for="(v, k, i) in value.shells.template"
      v-show="value.shells.template[k].show"
      class="v-t-col"
      :key="i"
      :style="value.style[k]"
      :data-column="k"
      :data-index="value.shells.template[k].order"
      :data-index-main="value.shells.template[k].order"
      @dragover="dragOver"
      @dragleave="dragLeave"
    >
      <div
        v-if="value.shells.template[k].hidden !== true"
        class="v-t-row"
        :data-index="value.shells.template[k].order"
      >
        <!-- label -->
        ...
        >
          ...
          >
            ...
            <b-dropdown-group
              v-if="value.shells.template[k].sortable"
            >
              <b-dropdown-item
            ...
            <b-dropdown-item
              @click="value.shells.template[k].show=false"
              variant="danger"
  ...
  export default {
    ...
    computed:{
      ...
      tableOptionsIsInitial(){
        _.forEach(this.value.shells.template, i=>{
...
console.log('add')
      ...
      dragChangeEnd(){
        ...
        Object.keys(this.value.shells.template).map((k)=>{
          if(this.value.shells.template[k].order===this.value.drag.from) {this.value.shells.template[k].order=this.value.drag.before-1}
          else if(this.value.shells.template[k].order<this.value.drag.before) this.value.shells.template[k].order--
        });
      },
      dragChangeStart(evt){
      ...
      dragResize: _.throttle( function(evt){
          if(evt.clientX !== 0)
          {
            ...
            this.$set(this.value.shells.template[this.value.drag.column], 'style',  `min-width: ${width}px; max-width: ${width}px;`);
