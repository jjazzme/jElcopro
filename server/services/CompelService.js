...
export default class CompelService {
    ...
    async #parseApiItem(item, case_, store) {
        ...
        const newItem = item.proposals[0].location_id !== 'CENTRE' ? {} :
            {
                ballance: this.#getQuantity(item.proposals[0]),
    ...
    async parseApiAnswer(result, days = 6) {
        ...
        const items = this.#clearItems(result.items);
            ...
            items.map(item => new Promise(async (resolve, reject) => {
                    ...
                    resolve([].concat(...item.proposals.map( proposal => proposal.price_qty.map( price => {
                        return  {
                            ...
                            good_id: good.id,
                            ballance: this.#getQuantity(proposal),
                            min: price.min_qty,
                            max: price.max_qty === 0 ? this.#getQuantity(proposal) : price.max_qty,
